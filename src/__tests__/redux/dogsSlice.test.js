...
describe('dogsSlice actions', () => {
  ...
  test('should have changeLifeSpanFilter action that creates a lifeSpanFilter proprety in the filters object of the state', () => {
    store.dispatch(changeLifeSpanFilter('young'));
    const { filters } = store.getState().dogs;

    expect(filters.lifeSpanFilter).toBeDefined();
    expect(filters.lifeSpanFilter).toStrictEqual('young');
  });
